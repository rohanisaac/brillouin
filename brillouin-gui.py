...
import numpy as np
import matplotlib
matplotlib.use('WxAgg')  # otherwise tk stuff crashes
...
def plotfit(obj, ndir, fname):
    ...
    plt.savefig(os.path.join(ndir, fname))
    plt.close()
...
@Gooey(program_name='Brillouin data process', required_cols=1, optional_cols=1)
def main():
    parser = GooeyParser(description='Brillouin processing script')
    parser.add_argument(
        'input_dir',
    ...
    print 'Procssing data in {} with spacing {} cm, peaks crossed: {}'.format(args.input_dir, args.spacing, args.crossed)
    ...
    wfil = open(os.path.join(folname, 'output.csv'), 'w')
    for f in os.listdir(folname):
        ...
        filx = fil.split('.')[0]
        ndir = os.path.join(bdir, filx)
        if not os.path.exists(ndir):
            os.makedirs(ndir)

        # redirect all console out to log file
        sys.stdout = open(os.path.join(ndir, 'output.txt'), 'w')
        print(fname)
