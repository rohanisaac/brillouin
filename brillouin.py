...
def plot_fit(l, b1, b2, save_path, filename, spacing=0.56, crossed=False):
        ...
        for pa, pb in [(l1, p1), (l2, p2), (l2, p3), (l3, p4)]:
            ax.annotate("",
                        ...
                        arrowprops=dict(arrowstyle="<|-|>",
                                        connectionstyle="arc3", color='gray', alpha=0.7),
                        )
    ...
    handles = [mlines.Line2D([], [], ls='--', c='gray', lw=1, label='Brillouin peak'),
               mlines.Line2D([], [], ls='-', c='gray', lw=2, label='Laser peak'),
    ...
    plt.title('Filename: {}, Spacing: {:.2} cm, {:.5f} wn/ch'.format(filename, spacing, wn_ch), y=-0.30, bbox=dict(facecolor='gray', alpha=0.3))
    plt.legend(handles=handles, loc='lower center', bbox_to_anchor=(0.5, 1.05), ncol=4)
